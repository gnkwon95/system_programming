...
void non_dealloc(){
	...
	while (next) {
		next = tmp->next;
		LOG_BLOCK(tmp->ptr, tmp->size, tmp->cnt, "???", tmp->ofs);
		tmp = next;
	}
...
//	LOG_REALLOC(ptr, size, 
	...
	if (findfree != NULL){
		...
		// if reallocated to different pointer, previous ptr is at freed
		if (tmp->ptr != p) {
		...
		alloc(list, p, size);
		return p;
	} else{ // illegal allocation
		item *tmp = find(freed, ptr);
		if (!mallocp){
			mallocp = dlsym(RTLD_NEXT, "malloc");
		}
		int* p = mallocp(size);	
